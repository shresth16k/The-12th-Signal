...
class FanSignal(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc), description="Time the signal was captured"
    )
    ...
    sentiment_score: float = Field(
        ..., ge=-1.0, le=1.0, description="Polarity score from -1.0 (very negative) to 1.0 (very positive)"
    )
...
class SignalCluster(BaseModel):
    ...
    topic: str = Field(
        ..., max_length=200, description="Identified topic/theme of the cluster (e.g., crowd congestion, temperature)"
    )
    confidence_score: float = Field(
        ..., ge=0.0, le=1.0, description="Clustering algorithm confidence level (0.0 to 1.0)"
    )
    created_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc), description="Time the cluster was created"
    )
...
class Consensus(BaseModel):
    ...
    contributing_opinions: List[AgentOpinion] = Field(
        ..., description="List of agent opinions reviewed to reach consensus"
    )
    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc), description="Time consensus was reached"
    )
...
class FanProfile(BaseModel):
    ...
    mobility_needs: Optional[str] = Field(
        None, max_length=500, description="Accessibility or mobility needs (e.g. wheelchair access)"
    )
    ...
    food_preferences: List[str] = Field(
        default_factory=list, description="Food restrictions or preferences (e.g. vegetarian, halal)"
    )
